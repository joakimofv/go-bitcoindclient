...
type GetIndexInfoResp struct {
	...
	// Key: name, Value: struct
	Map map[string]GetIndexInfoRespElement
}

func (alts GetIndexInfoResp) MarshalJSON() ([]byte, error) {
	return json.Marshal(alts.Map)
}

func (alts *GetIndexInfoResp) UnmarshalJSON(b []byte) error {
	reset := *alts
	var decoder *json.Decoder
	decoder = json.NewDecoder(bytes.NewReader(b))
	decoder.DisallowUnknownFields()
	...
	alts.Map = reset.Map
	return &UnmarshalError{B: b, structName: "GetIndexInfoResp"}
}

type GetIndexInfoRespElement struct {
...
type ValidateAddressResp struct {
	...
	// Element: N    index of a potential error
	ErrorLocations []float64 `json:"error_locations,omitempty"`
}
